...
function renderCase() {
  ...
  <div class="stats">${[['归并事件',a.summary.groups??a.summary.events,`${a.summary.events} 条原始证据`],['目标受害且成功',a.summary.success,'要求明确受害角色字段'],['远控类连接记录',a.summary.remote,'原始记录数，重复记录见归并事件'],['目标发起内网通信',a.summary.lateral,'通信源角色，不等于攻击成功']].map(([label,v,foot])=>`<div class="stat"><div class="stat-label">${label}</div><div class="stat-value">${v}</div><div class="stat-foot">${foot}</div></div>`).join('')}</div>
