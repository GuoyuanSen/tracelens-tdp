...
function renderOverview() {
  $('#main').innerHTML=heading('SECURITY INVESTIGATIONS','调查工作台',state.config.mode==='demo'?'当前使用合成演练数据，不连接真实环境。':'查询告警主机、核查关联日志并保存调查记录。',`<button class="button primary" data-action="new" ${state.config.ready?'':'disabled'}>＋ 发起主机调查</button>`)+(!state.config.ready?`
  ...
  ${state.config.mode==='demo'?`<section class="scenario-strip"><div class="scenario-index">CASE<br><strong>01</strong></div><div><div class="eyebrow">异常外连调查 · 合成演练</div><h2>演练案例：财务终端异常外连</h2><p>从 192.0.2.10 开始，依次核查钓鱼访问、远控连接与横向活动。</p></div><button class="button primary" data-action="new" data-ip="192.0.2.10">调查这台终端 →</button></section>`:''}<div class="toolbar panel" id="overview-range"><span class="muted">查询范围</span><select id="hours" aria-label="查询时间范围">${[[24,'过去 24 小时'],[72,'过去 3 天'],[168,'过去 7 天']].map(([v,t])=>`<option value="${v}" ${state.hours===v?'selected':''}>${t}</option>`).join('')}</select><span class="spacer"></span><button class="button small" data-action="refresh-overview">刷新数据</button></div>
...
function renderCase() {
  ...
  <div class="stats">${[['可用证据',a.summary.events,'当前快照保留的记录'],['攻击成功记录',a.summary.success,'TDP 标记，待核实影响'],['远控连接迹象',a.summary.remote,'出站远控类且连接为真'],['内网渗透告警',a.summary.lateral,'方向不等于攻击成功']].map(([label,v,foot])=>`<div class="stat"><div class="stat-label">${label}</div><div class="stat-value">${v}</div><div class="stat-foot">${foot}</div></div>`).join('')}</div>
  <div class="tabs" role="tablist">${[['overview','调查判断'],['timeline','证据时间线'],['network','通信关系'],['notes','人工记录'],['ai','AI 辅助分析']].map(([id,label])=>`<button class="tab ${state.tab===id?'active':''}" role="tab" aria-selected="${state.tab===id}" data-action="case-tab" data-tab="${id}">${label}</button>`).join('')}</div><div id="case-content"></div>`;
  renderCaseContent();
}
function renderCaseContent() {
  const c=state.case,snap=currentSnapshot(),a=snap.analysis;
  if(state.tab==='overview') {
    $('#case-content').innerHTML=`<div class="case-layout"><div class="stack"><section class="assessment-card"><div class="assessment-icon">◎</div><div><div class="eyebrow">查询结果摘要</div><h2>${esc(a.assessment)}</h2><p>依据 ${a.summary.events} 条记录整理 · 规则辅助判断 · 尚待人工确认</p></div></section><div class="investigation-progress"><span class="done">✓ 确定对象</span><i></i><span class="done">✓ 收集证据</span><i></i><span class="current">③ 核查判断</span><i></i><span>④ 记录与复查</span></div>${a.hypotheses.map(h=>`<section class="panel question"><div class="question-top"><h3>${esc(h.question)}</h3>${badge(h.state,h.evidence.length?'orange':'outline')}</div><p>${esc(h.finding)}</p><div class="refs">${h.evidence.slice(0,12).map(ref=>`<button class="ref" data-action="evidence" data-ref="${ref}">${ref} ↗</button>`).join('')}${h.evidence.length>12?`<span class="muted">另有 ${h.evidence.length-12} 条，见时间线</span>`:''}</div></section>`).join('')}<section class="panel"><div class="panel-header"><h2>待核实事项</h2></div><div class="panel-body"><ul class="checklist">${a.gaps.map(g=>`<li>${esc(g)}</li>`).join('')}</ul></div></section></div><aside class="stack"><section class="panel"><div class="panel-header"><h2>下一步调查</h2></div><div class="panel-body"><ul class="checklist">${a.next_steps.map(g=>`<li>${esc(g)}</li>`).join('')}</ul></div></section><section class="panel"><div class="panel-header"><h2>调查状态</h2></div><div class="panel-body"><select id="case-status" aria-label="调查状态">${Object.entries(statuses).map(([v,t])=>`<option value="${v}" ${c.status===v?'selected':''}>${t}</option>`).join('')}</select><button class="button small" data-action="save-status">更新状态</button><p class="subtitle">只更新本地档案，不修改 TDP 处置状态。归档不代表风险已消除。</p></div></section><section class="panel"><div class="panel-header"><h2>关联资产快照</h2></div><div class="panel-body">${assetCards(a)}</div></section></aside></div>`;
...
function peerModal(ip) {
  ...
  modal(ip+' · 关联证据',`<p class="subtitle">以下 ${pair[1].length} 条记录直接涉及该地址与当前调查主机。</p><div class="peer-evidence">${pair[1].slice(0,50).map(e=>`<div class="peer-evidence-row"><div><strong>${esc(e.name)}</strong><small>${stamp(e.time)} · ${esc(e.src_ip)} → ${esc(e.dest_ip)}</small></div><button class="ref" data-action="evidence" data-ref="${e.ref}">${e.ref} ↗</button></div>`).join('')}</div>${pair[1].length>50?'<p class="subtitle">此处显示前 50 条；其余见时间线。</p>':''}`);
}
function renderTimeline() {
  ...
  $('#timeline-items').innerHTML=events.length?`<div class="timeline">${events.map(e=>`<article class="timeline-item"><div class="time">${new Date(e.time*1000).toLocaleTimeString('zh-CN',{hour12:false})}<small>${new Date(e.time*1000).toLocaleDateString('zh-CN')}</small></div><div class="track-dot"></div><div class="timeline-content"><h3>${esc(e.name)}</h3><p><span class="ip">${esc(e.src_ip||'未知')}</span> → <span class="ip">${esc(e.dest_ip||'未知')}</span></p>${e.indicator?`<p class="wrap-anywhere">线索：${esc(e.indicator)}</p>`:''}<div class="timeline-tags">${severityBadge(e.severity)}${badge(directions[e.direction]||'方向未知')}${badge(e.result==='success'?'TDP 标记成功':e.result==='failed'?'TDP 标记失败':'结果待确认',e.result==='success'?'red':'outline')}<button class="ref" data-action="evidence" data-ref="${e.ref}">${e.ref} · 查看原始证据 ↗</button></div></div></article>`).join('')}</div>`:empty('没有匹配的证据','可调整筛选条件或调查时间范围。');
}
function evidenceModal(ref) {const e=currentSnapshot().analysis.evidence.find(e=>e.ref===ref);if(!e)return;modal(ref+' · 原始证据',`<div class="detail-grid"><div><label>事件</label>${esc(e.name)}</div><div><label>时间</label>${stamp(e.time)}</div><div><label>原始 ID</label><span class="wrap-anywhere">${esc(e.id)}</span></div><div><label>记录来源</label>${state.case.source.startsWith('demo:')?'合成案例演练':'TDP 日志调查 API'}</div></div><pre class="code">${esc(JSON.stringify(e.raw,null,2))}</pre><p class="subtitle">原始证据是外部数据，可能包含攻击者控制的内容。此处仅作为文本展示。</p>`);}
function recheckModal() {const last=state.case.snapshots.at(-1).analysis;const end=Math.floor(Date.now()/1000);modal('后续窗口复查',`<form id="recheck-form"><div class="callout"><p>请选择上次窗口结束后的时间。新快照会保留原始调查，不覆盖先前证据。</p></div><div class="form-grid">${inputField('开始时间','time_from',localInput(Math.ceil(last.time_to/60)*60),'datetime-local','','required')}${inputField('结束时间','time_to',localInput(end),'datetime-local','','required')}</div><p class="subtitle">如果刚完成调查，请在积累后续数据后再复查。没有新记录不能证明风险消失。</p><div class="form-footer"><button class="button primary" type="submit">查询并保存复查</button></div><p id="form-error" class="inline-error"></p></form>`);}
async function updateCase(data) {state.case=data;state.snapshot=data.snapshots.length-1;renderCase();}
async function previewAI() {const data=await api(`/api/cases/${state.case.id}/ai-preview`);state.aiPreview=data;modal('确认分析数据的去向',`<div class="callout warning"><p>以下摘要将发送到 <strong class="wrap-anywhere">${esc(data.endpoint)}</strong>。包含调查 IP、时间、源/目的 IP、威胁名称与 IOC；云端服务会接收到这些数据。</p></div><pre class="code">${esc(JSON.stringify(data.packet,null,2))}</pre><label class="check"><input id="ai-consent" type="checkbox">我确认可将上述证据摘要发送到该模型服务进行分析。</label>`,`<button class="button" data-action="close">取消</button><button class="button primary" data-action="ai-generate">发送并分析</button>`);}
